"""Visa-free spark-gap figure pack and report writers.

Imported by the acquisition CLI and by the analysis GUI. Do not import
instruments or PyVISA here.
"""
...
from lib.waveform import decimate_minmax, load_metadata, pick_time_scale, pick_voltage_scale
...
def write_analysis(
    ...
    written = [
        ...
        out_dir / f"{source.stem}_summary.json",
    ]
    write_events_csv(written[0], result.events)
    write_summary_json(written[1], result, source, run=run)
    if METRICS_SRC.exists():
        metrics = out_dir / "METRICS.md"
